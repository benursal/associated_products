...
{
	$obj =& get_instance();
	if($obj->uri->segment(1)=='student')
	$base_url = $obj->config->item('secure_base_url');
	else
		if($obj->uri->segment(1)=='inquiry' ||   $obj->uri->segment(1)=='registration')
		$base_url = $obj->config->item('secure_base_url');
		else
...
function image_asset($asset_name, $module_name = '', $attributes = array())
{
	...
	return '<img src="'.image_asset_url($asset_name, $module_name).'"'.$attribute_str.'></img>';
}
...
/**
  * Physical Base URL Helper
  *
  * Helps generate base physical url.
  *
  * @access		public
  * @param		string    the name of the file or asset
  * @param		string    optional, module name
  * @return		string    full physical url to image asset
  */
function base_url_physical_andy()
{
	$physical_base_path = explode('/', $_SERVER['SCRIPT_FILENAME']);
	$arr_base = array_splice($physical_base_path, 0, (count($physical_base_path)-1));
	return implode('/', $arr_base).'/';
}
/**
  * Physical Base URL Helper
  *
  ...
  * @return		string    full physical url to image asset
  */
function base_url_physical()
{
	return addslashes(FCPATH);
}
